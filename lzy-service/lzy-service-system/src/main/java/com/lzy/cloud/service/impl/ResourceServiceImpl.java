...
import com.lzy.cloud.mapper.ResourceMapper;
...
public class ResourceServiceImpl extends ServiceImpl<ResourceMapper, Resource> implements IResourceService {
    ...
    public boolean createResource(Resource resource) {
        ...
        LambdaQueryWrapper<Resource> resourceQueryWrapper = Resource.lqw();
        resourceQueryWrapper.eq(Resource::getResourceKey, resourceKey);
        long count = this.count(resourceQueryWrapper);
        if (count > 0) {
            throw new BusinessException("资源标识已存在");
        }

        if(null != resource.getParentId() && resource.getParentId().longValue() != LzyCloudConstant.PARENT_ID.longValue())
        {
        ...
        {
            resource.setAncestors(LzyCloudConstant.PARENT_ID.toString());
        }
    ...
    public boolean updateResource(Resource resource) {
                ...
                && null != resource.getParentId() && resourceOld.getParentId().longValue() != resource.getParentId().longValue())
        {
            ...
            {
                ancestorsNew = LzyCloudConstant.PARENT_ID.toString();
            }
            //设置组织新的父级组合
            ...
            //只查子节点
            resourceParent.setIsLeaf(LzyCloudConstant.Number.ONE);
    ...
    public Long queryResourceMaxId() {
        Long idMax = LzyCloudConstant.ZERO_LONG;
