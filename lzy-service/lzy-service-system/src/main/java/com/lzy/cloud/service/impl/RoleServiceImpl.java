...
import com.lzy.cloud.mapper.RoleMapper;
...
public class RoleServiceImpl extends ServiceImpl<RoleMapper, Role> implements IRoleService {
    ...
    @Override
    public Page<Role> selectRoleList(Page<Role> page,  QueryRoleDTO queryRoleDTO) {
        Page<Role> roleInfoList = roleMapper.selectRoleList(page, queryRoleDTO);
        return roleInfoList;
    }

    @Override
    public boolean createRole(CreateRoleDTO role) {
        LambdaQueryWrapper<Role> ew = Role.lqw();
    ...
    public boolean updateRole(UpdateRoleDTO role) {
        ...
        Role roleEntity = BeanCopierUtils.copyByClass(role, Role.class);
        QueryWrapper<Role> wrapper = Role.qw();
    ...
    public boolean deleteRole(Long roleId) {
        ...
        if (result) {
            // 删除用户和角色的关联关系
            LambdaQueryWrapper<UserRole> wr = UserRole.lqw();
            wr.eq(UserRole::getRoleId, roleId);
            userRoleService.remove(wr);
            // 删除角色和权限的关联关系
            LambdaQueryWrapper<RoleResource> rr = RoleResource.lqw();
    ...
    public boolean batchDeleteRole(List<Long> roleIds) {
        ...
        if (result) {
            // 删除用户和角色的关联关系
            LambdaQueryWrapper<UserRole> wrs = UserRole.lqw();
            wrs.in(UserRole::getRoleId, roleIds);
            userRoleService.remove(wrs);
            // 删除角色和权限的关联关系
            LambdaQueryWrapper<RoleResource> rrs = RoleResource.lqw();
