 ...
 */
@Data
@TableName("t_sys_data_permission_rule")
@ApiModel(value = "SysDataPermissionRule对象", description = "数据权限配置表")
public class SysDataPermissionRule extends BaseEntity implements Serializable {

    public static final LambdaQueryWrapper<SysDataPermissionRule> lqw(){
        return new LambdaQueryWrapper<>();
    }

    public static final LambdaUpdateWrapper<SysDataPermissionRule> luw(){
        return new LambdaUpdateWrapper<>();
    }

    public static final QueryWrapper<SysDataPermissionRule> qw(){
    ...
    @ApiModelProperty(value = "自定义数据权限（增加 where条件）")
